...
- 总师提出在仿真BOM内引入多维组织能力：保持现有产品结构视角，同时新增按时间（月度）与仿真类型的组织方式，并支持最多三维组合过滤，右侧内容布局与交互沿用现状。
...
## 需求拆解
1. **视角切换**
   - 需要提供 `结构 / 时间（月） / 类型` 三个视角，默认进入结构视角；方案BOM仍默认展示类型视角。
   - 视角切换控件与左侧树面板合并，整个仿真导航只保留一棵树：结构视角展示产品结构层级、时间视角为年份→月份两级、类型视角保持类型→实例→文件夹。
   - 各视角应保留独立的展开状态与滚动位置；切换视角时，根据当前组合筛选自动聚焦首个匹配实例，确保右侧内容不为空。
2. **组合过滤（最多三维）**
   - 支持结构+时间、结构+类型、时间+类型以及三者组合；组合维度上限提升为 3，超限提示“最多选择三种维度，先清除已有条件”。
   - 已选条件标签与清空入口放置在左树顶部，与视角切换同域展示，不影响状态/Owner/标签等细粒度过滤。
3. **时间维度**
